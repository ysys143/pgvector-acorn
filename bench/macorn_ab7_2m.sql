-- macorn_ab7_2m.sql : plain vs B3 at 2M, INDEPENDENT filter, SERIAL builds.
--
-- Downscale from the 10M attempt, which livelocked: 8 parallel workers spun on
-- the relation-extension LWLock (~2 blocks/min, never completes). SERIAL build
-- (max_parallel_maintenance_workers=0) has a single writer -> no extension-lock
-- contention -> steady progress. 2M is 10x the 200K reading, enough to show the
-- 200K->2M trend in B3's high-ef independent-filter advantage. M-ACORN dropped
-- (settled: inert). Truth = seqscan before indexes; queries held out (id>2M);
-- warm-up pass before timing; build_seed for reproducibility.
--
-- Run: nohup sudo -u postgres psql -d bench -f macorn_ab7_2m.sql > /tmp/ab7.log 2>&1 &
...
CREATE TABLE mab_queries AS
    ...
    FROM tv_items WHERE id > 2000000 ORDER BY id LIMIT 100;
...
-- SERIAL deterministic builds (no parallel -> no extension-lock livelock) ------
SET max_parallel_maintenance_workers = 0;
SET pg_acorn.build_seed = 42;

\echo >>> building plain index (payload_edges off, serial)
...
\echo >>> building B3 index (payload_edges on, payload_m 64, serial)
SELECT clock_timestamp() AS t \gset
...
\echo
\echo ====== plain vs B3 (2M, INDEPENDENT filter, serial, warm): recall@10 + latency ======
SELECT p.sel, p.ef,
