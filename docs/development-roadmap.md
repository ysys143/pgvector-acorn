...
5. **[완료] Tier 2 재개 스캔 (하이브리드)** (`acorn_scan.c` `acorn_stream_*` + `acorn_am.c`) — 중
   - 2단계: phase 1 = ef=40 beam batch(저비용·정확, 집중된 매치를 잡음), phase 2 = batch
     소진 시에만 streaming frontier로 **재탐색 없이 이어받기**(entry point부터 재시작 안 함).
     Tier 1 batch 경로는 불변(격리).
   - 결과(RESULTS.md Run 4): g1 @ 1% 63,000→**22,001** 페이지(2.9배↓), recall 1.000→0.998.
     g2 @ 1%는 **371 유지**(phase 1이 처리, 회귀 없음). 하이브리드가 old batch를 전 구간 지배.
   - 주: 순수 streaming은 best-first가 ef-beam의 방향성을 잃어 g2@1%에서 ~21,800으로 과확장 →
     phase 1을 둔 이유. 회귀 7/7 + 격리 2/2 + 단위 10/10 통과.
